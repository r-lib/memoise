...
By default, memoise uses an in-memory cache, using `cache_mem()` from the [cachem](https://r-lib.github.io/cachem/) package. `cachem::cache_disk()` allows caching using files on a local filesystem.
...
It is possible to use other caching backends with memoise. These caching objects must be key-value stores which use the same API as those from the [cachem](https://r-lib.github.io/cachem/) package. The following methods are required for full compatibiltiy with memoise:
